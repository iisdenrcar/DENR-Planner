...
function patchAsyncRoutes(app: any) {
  for (const method of ["get", "post", "put", "delete", "patch", "options"]) {
    const orig = app[method].bind(app);
    app[method] = (path: any, ...handlers: any[]) => {
...
function makeRateLimiter(opts: { windowMs: number; max: number; key: (req: any) => string }) {
  ...
  return (req: any, res: any, next: any) => {
    ...
    if (store.size > 5000) {
      for (const [kk, vv] of store) {
        if (now > vv.resetAt) store.delete(kk);
      }
    }
    next();
  };
...
const limitUpload = makeRateLimiter({
  windowMs: 10 * 60 * 1000,
  max: 120,
  key: (req) => `${req.ip}|upload|${String((req as any).user?.sub || "")}`
});

const loginFailStore = new Map<string, { firstAt: number; fails: number; lockUntil: number }>();
function normUserKey(username: any) {
  return String(username || "").trim().toLowerCase();
}
function checkLoginLock(username: any) {
  const k = normUserKey(username);
  const rec = loginFailStore.get(k);
  const now = Date.now();
  if (rec && rec.lockUntil > now) return { locked: true, retryAfterMs: rec.lockUntil - now };
  return { locked: false, retryAfterMs: 0 };
}
function noteLoginFailure(username: any) {
  const k = normUserKey(username);
  if (!k) return;
  const now = Date.now();
  ...
  base.fails += 1;
  if (base.fails >= maxFails) base.lockUntil = now + lockMs;
  loginFailStore.set(k, base);
  if (loginFailStore.size > 5000) {
    for (const [kk, vv] of loginFailStore) {
      if (vv.lockUntil > 0 && now > vv.lockUntil) loginFailStore.delete(kk);
      else if (now - vv.firstAt > windowMs && vv.lockUntil === 0) loginFailStore.delete(kk);
    }
  }
}
function clearLoginFailures(username: any) {
  const k = normUserKey(username);
  if (!k) return;
...
if (frontendUrl) defaultCorsAllowList.unshift(frontendUrl);
...
const adminStreamTokens = new Map<string, { user: any; expiresAt: number }>();

const twoFactorStore = new Map<string, { codeHash: string; expiresAt: number; attemptsLeft: number; lastSentAt: number }>();
...
function hashTwoFactorCode(username: string, code: string) {
...
function verifyPassword(stored: any, password: string): { ok: boolean; needsUpgrade: boolean } {
...
app.post("/api/login", limitLogin, async (req, res) => {
  await withUsersLock(async () => {
    ...
    if (lock.locked) return res.status(429).json({ error: "account_locked", retryAfterMs: lock.retryAfterMs });
    ...
    const pw = verifyPassword((user as any).password, String(password ?? ""));
    if (!pw.ok) {
      noteLoginFailure(username);
    ...
    if (pw.needsUpgrade) {
      (user as any).password = hashPassword(String(password ?? ""));
      await (await import("./storage-select.js")).writeUsers(users);
    }
    const isAdmin = String((user as any).role || "").includes("ADMIN");
    if (isAdmin) {
...
app.post("/api/login/start", limitLogin, async (req, res) => {
  await withUsersLock(async () => {
    ...
    if (!isEmailLike(email)) return res.status(400).json({ error: "email_required_for_2fa" });
...
app.post("/api/login/verify", limitVerify, async (req, res) => {
  ...
  if (typeof tempToken !== "string" || !tempToken.trim()) return res.status(400).json({ error: "missing_temp_token" });
  if (typeof code !== "string" || !/^\d{6}$/.test(code.trim())) return res.status(400).json({ error: "invalid_code_format" });

  const decoded = verifyToken(tempToken.trim());
  if (!decoded || !(decoded as any).twoFactor || typeof (decoded as any).sub !== "string") {
    return res.status(401).json({ error: "invalid_temp_token" });
  }
  const username = String((decoded as any).sub);
  const record = twoFactorStore.get(username);
  if (!record) return res.status(410).json({ error: "two_factor_expired" });
  const now = Date.now();
  ...
  const token = signToken({
    ...
    officeName: (decoded as any).officeName,
    service: (decoded as any).service
  });
  res.json({ token });
});
...
function normalizeHolidaysList(input: any[]): Array<{ month: number; day: number; name?: string }> {
  const map = new Map<string, { month: number; day: number; name?: string }>();
  for (const h of Array.isArray(input) ? input : []) {
    const m = Number((h as any)?.month);
    const d = Number((h as any)?.day);
    if (!Number.isFinite(m) || !Number.isFinite(d)) continue;
    if (m < 1 || m > 12 || d < 1 || d > 31) continue;
    const key = `${m}-${d}`;
    const name = typeof (h as any)?.name === "string" ? (h as any).name : undefined;
...
app.get("/api/holidays", async (_req, res) => {
  ...
  res.json(normalizeHolidaysList(holidays as any));
});
...
app.post("/api/admin/backup/restore", authMiddleware, checkUserDisabled, requireAnyRole(["ADMIN"]), async (req, res) => {
  ...
  if (!loaded) return res.status(400).json({ error: "missing_backup" });
...
app.post("/api/admin/stream-token", authMiddleware, checkUserDisabled, requireAnyRole(["ADMIN"]), async (req, res) => {
  ...
  adminStreamTokens.set(streamToken, { user: (req as any).user, expiresAt: Date.now() + ttlMs });
...
setInterval(() => {
  ...
  for (const [k, v] of adminStreamTokens) {
    if (now > v.expiresAt) adminStreamTokens.delete(k);
  }
}, 60 * 1000);
...
app.post("/api/admin/holidays", authMiddleware, checkUserDisabled, requireAnyRole(["ADMIN"]), async (req, res) => {
  try {
    await withHolidaysLock(async () => {
      ...
      const holidays = await readHolidays();
      const normalized = normalizeHolidaysList(holidays as any);
      ...
      const next = normalizeHolidaysList([...(normalized as any), holiday] as any);
...
app.delete("/api/admin/holidays/:month/:day", authMiddleware, checkUserDisabled, requireAnyRole(["ADMIN"]), async (req, res) => {
  try {
    await withHolidaysLock(async () => {
      ...
      await writeHolidays(normalizeHolidaysList(remaining as any));
      const actor = (req as any).user as any;
...
app.put("/api/admin/holidays/:month/:day", authMiddleware, checkUserDisabled, requireAnyRole(["ADMIN"]), async (req, res) => {
  try {
    await withHolidaysLock(async () => {
      ...
      const holidays = normalizeHolidaysList(holidaysRaw as any);
      ...
      await writeHolidays(normalizeHolidaysList(holidays as any));
...
async function runReminderScheduler() {
  try {
    await withEventsLock(async () => {
      ...
      for (const ev of events) {
        const currentMask = (() => {
          const n = Number((ev as any).reminderSentMask);
          if (Number.isFinite(n)) return n;
          return (ev as any).reminderSent ? REMINDER_3D : 0;
        })();
        ...
        if (nextMask !== currentMask) {
          (ev as any).reminderSentMask = nextMask;
          (ev as any).reminderSent = (nextMask & REMINDER_3D) === REMINDER_3D;
          mutated = true;
        }
      }
    ...
    await withUsersLock(async () => {
      ...
      for (const u of users) {
        ...
        if (typeof pw === "string" && pw && !pw.startsWith("scrypt$")) {
          (u as any).password = hashPassword(pw);
          mutated = true;
        }
      }
      if (mutated) {
    ...
    return await withUsersLock(async () => {
      return await withHolidaysLock(async () => {
        ...
        for (const extra of files.slice(keep)) {
          try { fs.unlinkSync(path.join(backupDir, extra.f)); } catch {}
          try {
...
async function runPeriodicBackup() {
  try {
    ...
    if (result && (result as any).ok) {
      console.log("Backup saved:", (result as any).file);
    }
  } catch (err) {
...
app.use((err: any, _req: any, res: any, _next: any) => {
  ...
  if (!res.headersSent) {
    if (msg === "CORS blocked") return res.status(403).json({ error: "cors_blocked" });
    if (msg && /file too large/i.test(msg)) return res.status(413).json({ error: "file_too_large" });
    if (msg === "unsupported_file_type") return res.status(415).json({ error: "unsupported_file_type" });
